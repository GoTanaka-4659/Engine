...
void Model::StaticInitilize(ID3D12Device *device)
{
	assert(!Model::device);
...
void Model::LoadFromOBJInternal()
{
	...
	const string modelname = "ship";
	const string filename = modelname + ".obj";//"triangle_mat.obj"
	const string mtlname = modelname + ".mtl";//"triangle_mat.mtl"
	const string directoryPath = "Resources/";//"Resources/triangle_mat/"
	...
	while (getline(file, line))
	{
		...
		if (key == "mtllib")
		{
			...
			//マテリアル読み込み
			LoadMaterial(directoryPath, mtlname);
		}
		//先頭文字列がvなら頂点座標
		if (key == "v")
		{
